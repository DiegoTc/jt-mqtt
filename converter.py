...
class JT808Server:
    ...
    def _publish_batch_location(self, device_id, message):
        ...
        try:
            ...
            # Check if this exact payload was already published for this device+topic
            if device_topic_key in self._last_published_payload and self._last_published_payload[device_topic_key] == payload_str:
                logger.warning(f"Duplicate batch location detected for {device_id} on topic {topic}, skipping publication")
                return
    ...
    def _publish_logout(self, device_id):
        ...
        # Check if this exact payload was already published for this device+topic
        if device_topic_key in self._last_published_payload and self._last_published_payload[device_topic_key] == payload_str:
            logger.warning(f"Duplicate logout message detected for {device_id} on topic {topic}, skipping publication")
            return
    ...
    def _publish_status(self, device_id, status):
        ...
        if should_publish:
            ...
            # CRITICAL FIX: Check for duplicate status messages too
            if not hasattr(self, '_last_published_payload'):
                self._last_published_payload = {}
            ...
            # Check if this exact payload was already published for this device+topic
            if device_topic_key in self._last_published_payload and self._last_published_payload[device_topic_key] == payload_str:
                logger.warning(f"Duplicate status message detected for {device_id} on topic {topic}, skipping publication")
                return
